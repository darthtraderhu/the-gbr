...
// Az összehasonlítás alapja: egy átlagos magyar céges weboldal tipikus
// betöltési adatai. Nem a saját mérésünk átlaga — ha lesz erre valós,
// hivatkozható forrásunk (pl. HTTP Archive magyar domain-adatok), ezt
// érdemes lesz lecserélni és lábjegyzetelni.
...
const GAUGES: { key: GaugeKey; label: string; unit?: string; avgLabel: string }[] = [
  { key: "load", label: "Betöltés", unit: "ms", avgLabel: "Átlag 2 400" },
  { key: "ttfb", label: "Szerver válasz", unit: "ms", avgLabel: "Átlag 800" },
  { key: "kb", label: "Letöltött adat", unit: "kB", avgLabel: "Átlag 2 300" },
  { key: "req", label: "Hálózati kérés", avgLabel: "Átlag 74" },
];
  ...
  useEffect(() => {
    ...
    function animateCount(key: GaugeKey, target: number) {
      ...
      function frame(x: number) {
        ...
        if (el) el.textContent = Math.round(target * eased).toLocaleString("hu-HU");
    ...
    function measure() {
      ...
      if (verdictRef.current) {
        ...
        verdictRef.current.innerHTML =
          `Ez az oldal <b style="color:var(--signal)">${loadMultiplier}×</b> gyorsabban töltődött be nálad, mint egy átlagos magyar céges weboldal — és ` +
          `<b style="color:var(--signal)">${kbMultiplier}×</b> kevesebb adatot töltött le hozzá. Nem trükk: kevesebb fölösleges dolog van benne.`;
      }
      if (tsRef.current) {
        tsRef.current.textContent = new Date().toLocaleTimeString("hu-HU");
      }
      if (uaRef.current) {
        const m = navigator.userAgent.match(/(Chrome|Firefox|Safari|Edg)/);
        uaRef.current.textContent = m ? m[0].replace("Edg", "Edge") : "a te eszközöd";
      }
    }
