        ...
        <div className="absolute bottom-16 right-0 w-[320px] sm:w-[380px] bg-[#050505] border border-white/20 rounded-xl shadow-[0_0_40px_rgba(231,255,0,0.15)] overflow-hidden flex flex-col transform transition-all duration-300 origin-bottom-right">
          ...
          <div className="bg-[#0a0a0a] border-b border-white/10 px-4 py-3 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-gray-300 uppercase tracking-widest font-bold">Sys: GBR_AI_NODE</span>
            </div>
          ...
          <div className="h-[350px] p-4 overflow-y-auto flex flex-col gap-4 bg-cubes-pattern bg-opacity-5 relative">
            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`relative z-10 flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] p-3 rounded text-xs leading-relaxed ${msg.sender === "user" ? "bg-white/10 text-white border border-white/5 rounded-br-none" : "bg-[#e7ff00]/10 text-[#e7ff00] border border-[#e7ff00]/20 rounded-bl-none"}`}>
                  {msg.sender === "ai" && <div className="text-[8px] font-black uppercase tracking-widest mb-1 opacity-50">GBR_AI</div>}
                  {msg.text}
                </div>
              </div>
            ))}
            ...
            {isTyping && (
              <div className="flex justify-start relative z-10">
                <div className="bg-[#e7ff00]/10 border border-[#e7ff00]/20 p-3 rounded rounded-bl-none flex gap-1 items-center">
                  <div className="w-1.5 h-1.5 bg-[#e7ff00] rounded-full animate-bounce" style={{ animationDelay: "0ms" }}></div>
                  <div className="w-1.5 h-1.5 bg-[#e7ff00] rounded-full animate-bounce" style={{ animationDelay: "150ms" }}></div>
                  <div className="w-1.5 h-1.5 bg-[#e7ff00] rounded-full animate-bounce" style={{ animationDelay: "300ms" }}></div>
                </div>
              </div>
            )}
          ...
          <form onSubmit={handleSendMessage} className="border-t border-white/10 bg-[#0a0a0a] p-3 flex gap-2">
            <input 
              type="text" 
              value={inputValue}
            ...
            />
            <button type="submit" disabled={!inputValue.trim() || isTyping} className="p-2 bg-[#e7ff00] text-black rounded hover:bg-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
